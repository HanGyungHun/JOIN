      ...
      <div class="from-content">
        <div class="form_list">
          ...
          <div class="form_id" :class="{ 'error': !validUsername }">
            ...
            <p v-show="!success" class="input-success">{{ usernameSuccessMessage }}</p>
          </div>
        ...
        <div class="form_list">
          ...
          <div class="form_mail" id="mail" :class="{ 'error': !validEmail }">
            <img :src="imageData.email" class="email_icon">
            <input type="text" class="input" id="mail" name="mail" placeholder="이메일" autocomplete="off" v-model="email" @blur="validateEmail">
            <p v-show="!validEmail" class="input-error">{{ emailErrorMessage }}</p>
          </div>
...
export default {
  ...
  methods: {
    ...
    updateSelectedMBTI(value) {
      if (value === '직접 입력') {
        this.customMBTI = ''; // 직접 입력을 선택한 경우, 직접 입력 값을 초기화합니다.
      } else {
        this.customMBTI = value; // 선택 가능한 셀렉트 박스에서 값을 직접 입력 칸으로 전달합니다.
      }
    },
    ...
    axios.post('http://127.0.0.1:5000/duplicate',form)
        .then(response=>{
          if(response.data)
            ...
            this.usernameSuccessMessage = '∙ 사용 가능한 아이디 입니다.';
            console.log('오냐?')
          } 
        })
